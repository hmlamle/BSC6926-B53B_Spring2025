...
df = df |> 
  ...
  mutate(across(c(TT:sg_rich), scale), 
         # add tiny amount so when all values the same can make hv. This allows us to put in values for a data point that might have one value for all axes (like if we didn't have data for that year)     
         across(c(TT:sg_rich), 
...
df_y = df_y[!duplicated(t(apply(df_y,1,sort))),] %>% # this is organizing them by year and gets rid of those that are mirrored opposites
  filter(!(y1 == y2))  # this is not including comparisons that are comparing the same year. This doesn't tell us anything 
...
df_ov = tibble(BASIN = rep(unique(df$BASIN),
  ...
  mutate(ychange = y2-y1,  # this is the statistic that we are comparing to! Allows us to keep track of how many years between the comparison of HV's we are doing is
...
# shows RKB had highest overlap, so less variability, and more stability over time 
...
# shows that higher centroid distance is more different in mean conditions. JON has most centroid distance, so most different. Dashed line is standard deviations away from mean conditions. 
...
# shows JON and BLK are getting more heterogeneous over time, but RAN is getting more homogeneous over time
...
# year to year comparison, high overlap means that things aren't really changing. In RAN and WHP, where seagrass dieoff was in 2015, in 2016 overlap was 0 because that community completely changed 
...
# change relative to 2007: high overlap means similar, low overlap means not similar. JON got similar in the begining, then at 2016 on shifted and maybe got to alternative stable state because overlap is 0. 
...
# in context of stability, centroid distance is most useful and behaves a bit more intuitively. JON things were stable (centroid distance low, then gets higher). 
...
# size changes relative to 2007: RAN got smaller so HV size decreasing and everything got more homogeneous. 
...
df_o = df_ov |> 
  ...
  mutate(m_int = map(data, \(df)lm(sorensen~1, data = df)),
         ...
         model = case_when(
           ...
           T ~ 'Quadratic'))
# AICC is a parameter that allows us to compare model fit between models. If within 4, two models are indiscriminate between each other to describe the data. 
# parsimony - we chose 3 models (line, linear, quadratic), yeahhhh I didn't get this. Comparing these three models, each has different variables (line has 1, linear has 2, quadratic has 3), and comparing it to the AICC value of above or below 4. 
...
# for TWN - they are decreasing but decreasing consistently in space, so getting more homogeneous. 
# So this graph is telling us 2 things: The trend over time in comparison to starting year, and also the rate of change. a straight line is integral of linear change. A linear line is actually increasing exponentially. 
