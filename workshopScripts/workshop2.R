...
# Ryan built this custom function to get values easily. Can set default values
# (=NULL, etc). Everything else you have to input yourself. So this takes a dataframe
# that has rows, names, mean, etc and it will give you the other things (if it can)
...
set.seed(14) # reproducible random data (so it will give you the same random variables every time)
df = d |> 
  # duplicate for number of reps
  slice(rep(1:n(), each=reps))|>       # funciton allows you to change data set (subset or make longer). We repeated each value 3 times. This allows you to nest and map things. 
  mutate(i = rep(1:reps, times=nrow(d))) |>    # differentiates each of the rows that we are repeating (1-3)     
  group_by(i) |>            # grouping by iteration
  nest() |> 
...
df = df |> 
  group_by(species, season, i) |>  # creating breakdown of data we want to input and create HV's for. Creating HV for each species, during each season, and each iteration
  nest() |> 
  mutate(hv = map(data, \(data) hypervolume_gaussian(data, name = paste(species, season, i,sep = '_'),
                                                     ...
                                                     chunk.size = 1000, 
                                                     verbose = F)),
         hv_size = map_dbl(hv, \(hv) get_volume(hv)),  # extracting size and centroid from the HV
         centroid = map(hv, \(hv) get_centroid(hv)))

# after running above HV ^^, get more columns in the df for HV itself, HV size (as a number) and HV centroid (as a number)

# df1 = df
#write_rds(df, 'data/CESIhvAll.rds', compress = 'gz')

# overlap within species by season 
# takes our df with the hv's, just selects species, season, hv, hv_size, and i and pivots it so that the season (wet vs dry) is a column 
ov_sn = df |> 
  select(species, season, hv, hv_size, i) |> 
  pivot_wider(names_from = season, values_from = c(hv,hv_size)) |> #take data and pivot so season column becomes multiple columns with separate hv's for each species and season
  mutate(size_rat = hv_size_Dry/hv_size_Wet,
         # ^ ratio of dry vs wet so greater than 1 means that hv size in dry season > wet, and vice versa
         set = map2(hv_Wet,hv_Dry, \(hv1, hv2) hypervolume_set(hv1, hv2, check.memory = F, verbose = F)),
         # ^ use map2 to input wet and dry HV's to make a set (combination) to calculate overlap below
        ...
        # ^ map2_dbl compares 
  unnest_wider(ov) |> # takes info in a column and unnests it (unnest wider creates multiple columns from one column). Unnest longer makes multiple rows 
...
# this plot is showing niche overlap per species between seasons. 
# 3 different response levels of either lots of overlap, some, or none
...
# results for this for the bay anchovy and silver perch is that the wet season niche is constrained pretty much completely within it's dry season niche. 
...
# hammering home the previous plot, showing niche width shifts and showing that 
# bay anchovy and silver perch are showing a contraction/expansion of niches while 
# the other species are shifting their niches 
...
# and last, showing just how different these are in centroid distance. The dashed line 
# is one standard deviation away from the mean, so Bay anchovy and silver perch 
# have similar-ish centroid distances, showing not so much a shift away from mean but contracting/
# expanding of niche sizes. Neat!! 
